    ...
    app.run(debug=True, port=5000, host="127.0.0.1")
        ...
        if TF_AVAILABLE and os.path.exists(lstm_path):
            try:
                ...
                print("Could not load LSTM model:", e)
...
# ---------------------------------------------------------------------------
# Feature engineering for a single "live" order coming from the UI
# ---------------------------------------------------------------------------
def build_feature_row(payload):
    row = {f: 0.0 for f in (bundle.feature_names or [])}

    price = float(payload.get("product_price", 0) or 0)
    qty = float(payload.get("order_item_quantity", 1) or 1)
    discount_rate = float(payload.get("discount_rate", 0) or 0) / 100.0

    numeric_values = {
        "shipping_days_scheduled": float(payload.get("shipping_days_scheduled", 4) or 4),
        "order_item_quantity": qty,
        "product_price": price,
        "sales": price * qty,
        "order_item_discount": price * qty * discount_rate,
    ...
    if payload.get("disaster_toggle"):
        numeric_values.update({
            "active_disaster_count": float(payload.get("disaster_count", 1) or 1),
            "known_disaster_count": float(payload.get("disaster_count", 1) or 1),
            "current_max_disaster_magnitude": float(payload.get("disaster_magnitude", 5) or 5),
            "current_max_affected": float(payload.get("disaster_affected", 10000) or 10000),
            "current_max_deaths": float(payload.get("disaster_deaths", 0) or 0),
        })
    else:
        numeric_values.update({
            "active_disaster_count": 0.0, "known_disaster_count": 0.0,
            "current_max_disaster_magnitude": 0.0,
            "current_max_affected": 0.0, "current_max_deaths": 0.0,
        })
...
def top_factors(n=6):
    ...
    pairs = [p for p in pairs if p[1] > 0][:n]
    total = sum(p[1] for p in pairs) or 1.0
    return [{"feature": f, "share": round(float(v) / total * 100, 1)} for f, v in pairs]


def safety_stock_days(delay_days):
    """Very simple rule-based buffer suggestion (Chương 3.9 - Inventory Optimization)."""
    return round(max(0.0, delay_days) * 1.5 + 1, 1)
